<div class="space-y-8">
    ...
    @if(Auth::user()->isInstitutionAdmin())
        <!-- Institution Demographics Management (for Institution Admins) -->
        <div>
            <livewire:profile.institution.set-institution-demographic />
        </div>
    @else
        <!-- Regular User Demographic Tags -->
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <form wire:submit.prevent="saveTags" class="space-y-4">
                ...
                {{-- Institution-specific tags section for regular users --}}
                @if(Auth::user()->institution_id && count($institutionTagCategories ?? []))
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h3 class="text-lg font-semibold mb-3">{{ Auth::user()->institution->name ?? 'Institution' }} Demographics</h3>
