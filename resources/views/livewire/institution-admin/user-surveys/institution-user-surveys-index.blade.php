<div>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <div class="flex space-x-2 flex-wrap">
                            ...
                            <button wire:click="filterByStatus('ongoing')" 
                                class="px-4 py-2 text-sm rounded {{ $statusFilter === 'ongoing' ? 'bg-purple-600 text-white' : 'bg-gray-200' }}">
                                Ongoing ({{ $ongoingCount }})
                            </button>
                            <button wire:click="filterByStatus('finished')" 
                                class="px-4 py-2 text-sm rounded {{ $statusFilter === 'finished' ? 'bg-gray-600 text-white' : 'bg-gray-200' }}">
                            ...
                            <button wire:click="filterByStatus('archived')" 
                                class="px-4 py-2 text-sm rounded {{ $statusFilter === 'archived' ? 'bg-gray-400 text-white' : 'bg-gray-200' }}">
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-6">
                                        <div class="flex items-center space-x-1">
                                            @if($survey->trashed())
                                                <span class="px-2 py-1 rounded text-xs bg-gray-400 text-white">
                                                    Archived
                                                </span>
                                            @else
                                                @if($survey->status === 'pending')
                                                    <span class="px-2 py-1 rounded text-xs bg-yellow-200 text-yellow-800">Pending</span>
                                                @elseif($survey->status === 'published')
                                                    <span class="px-2 py-1 rounded text-xs bg-green-200 text-green-800">Published</span>
                                                @elseif($survey->status === 'ongoing')
                                                    <span class="px-2 py-1 rounded text-xs bg-purple-200 text-purple-800">Ongoing</span>
                                                @elseif($survey->status === 'finished')
                                                    <span class="px-2 py-1 rounded text-xs bg-gray-200 text-gray-800">Finished</span>
                                                @else
                                                    <span class="px-2 py-1 rounded text-xs bg-gray-200 text-gray-800">{{ ucfirst($survey->status) }}</span>
                                                @endif
                                            @endif
                                        ...
                                        <button 
                                            ...
                                            @click="
                                                $wire.set('selectedSurveyId', null).then(() => {
                                                    ...
                                                    $nextTick(() => $dispatch('open-modal', { name: 'institution-user-survey-view-modal' }));
                                                })
                                            "
    ...
    <x-modal name="institution-user-survey-view-modal" title="Survey Details">
        <div class="p-6 relative min-h-[400px] flex flex-col">
            ...
            <div wire:loading.remove class="flex-1">
                @if($selectedSurveyId)
                    @livewire('institution-admin.user-surveys.modal.institution-user-survey-view-modal', ['surveyId' => $selectedSurveyId], key('survey-modal-' . $selectedSurveyId))
                @else
