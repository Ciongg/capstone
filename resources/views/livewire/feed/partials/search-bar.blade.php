{{-- Top Bar - Changed to always be a row --}}
<div class="flex flex-row justify-between items-center gap-2 mb-4 sm:mb-6">
    {{-- Search Bar with Icons on both sides - Smaller width on mobile --}}
    <div class="relative w-3/5 sm:w-auto sm:max-w-md flex-grow">
        {{-- Left search icon --}}
        ...
        <input
            ...
            class="w-full pl-10 pr-10 py-2 rounded-full border border-gray-300 focus:border-blue-400 focus:outline-none bg-white shadow-md"
        />
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-3">
            ...
            {{-- Filter button (hidden while loading) --}}
            <button
                wire:loading.class="hidden"
                wire:target="search"
                @click="$wire.toggleFilterPanel()"
    ...
    {{-- User Points Display - Always on right side --}}
    <div class="flex flex-row items-center justify-end w-2/5 sm:w-auto py-2 px-4 bg-white rounded-full shadow-md">
        <span class="text-xl sm:text-4xl text-[#FFB349] font-bold">{{ auth()->user()->points ?? 0 }}</span>
        <svg class="w-5 h-5 sm:w-8 sm:h-8 text-[#FFB349] ml-1 sm:ml-2" viewBox="0 0 24 24" fill="currentColor">
