<div class="flex flex-col lg:flex-row lg:space-x-6 space-y-4 lg:space-y-0 p-2 sm:p-4 h-full">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500">
                    @if($survey->end_date)
                        ...
                        @endphp
                        <div>Ends: {{ $endDate->format('M j, Y g:i A') }}</div>
                        <div>Now: {{ $now->format('M j, Y g:i A') }}</div>
                    @else
                        @php
    ...
    <div class="flex flex-col space-y-3 lg:space-y-4 w-full lg:w-1/2 h-auto lg:h-full">
        ...
        <div class="flex flex-col items-center sm:items-end flex-shrink-0">
            ...
            @elseif($survey->is_not_started_locked ?? false)
                <!-- Locked due to survey not started yet -->
                <button 
                    class="flex items-center w-full sm:w-auto justify-center px-4 sm:px-6 py-3 sm:py-2 bg-gray-300 text-gray-600 font-semibold rounded-lg cursor-not-allowed text-sm sm:text-base"
                    disabled
                >
            ...
            @elseif($survey->is_response_limit_locked ?? false)
                <!-- Locked due to response limit reached -->
                <button 
                    class="flex items-center w-full sm:w-auto justify-center px-4 sm:px-6 py-3 sm:py-2 bg-gray-300 text-gray-600  font-semibold rounded-lg cursor-not-allowed text-sm sm:text-base"
                    disabled
                >
            ...
            @else
                ...
                <button 
                    type="button"
                    wire:click="showDataPrivacyNotice"
                    wire:loading.attr="disabled"
                    class="w-full sm:w-auto text-center px-4 sm:px-6 py-3 sm:py-2 bg-[#03b8ff] text-white font-bold rounded-lg hover:bg-[#0295d1] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150 text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed sm:min-w-[160px]"
                >
                    <span wire:loading.remove wire:target="showDataPrivacyNotice" class="inline-block">Answer Survey</span>
                    <span wire:loading wire:target="showDataPrivacyNotice" class="flex items-center justify-center">
