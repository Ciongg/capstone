...
<head>
    ...
    <style>
        ...
        body.no-scroll {
            ...
            position: fixed;
            width: 100%;
            /* height is not needed and can cause issues */
        }
...
">
                ...
                <div class="hidden lg:flex lg:items-center lg:justify-between lg:w-full lg:pl-6">
                            ...
                            @else
                                ...
                                @if(Auth::user()->isResearcher())
                                    ...
                                    <button 
                                        ...
                                        class="{{ $navLinkClass(request()->is('surveys/create*')) }} cursor-pointer hover:text-[#03b8ff] hover:underline"
                                ...
                                @elseif(Auth::user()->isSuperAdmin())
                                    ...
                                    <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                                        <button 
                                            @click="open = !open" 
                                            class="{{ $navLinkClass(request()->is('admin/*')) }} cursor-pointer hover:text-[#03b8ff] hover:underline flex items-center"
                                        >
                                            Manage
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    ...
                    <div class="flex items-center space-x-5 text-base">
                        @auth
                            ...
                            <button
                                x-on:click="$dispatch('open-modal', { name: 'announcement-carousel-modal' })"
                                class="cursor-pointer text-gray-700 hover:text-[#03b8ff] focus:outline-none relative -top-0.5"
                                title="Announcements"
                                aria-label="Open announcements"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="cursor-pointer w-8 h-8">
    ...
    <!-- Mobile Menu Section (slides in from right) - NOW OUTSIDE NAV -->
    <div 
        ...
        class="fixed top-0 right-0 h-full w-4/5 mobile-menu shadow-lg z-[9999] p-6 overflow-y-auto"
        style="display: none;"
        @keydown.escape.window="toggleMenu()"
    >
        ...
        <div class="flex flex-col space-y-4 text-lg pt-4">
            ...
            @else
                ...
                {{-- Mobile Menu: Announcements Button --}}
                <button
                    @click="mobileMenuOpen = false; $dispatch('open-modal', { name: 'announcement-carousel-modal' })"
                    class="flex items-center text-gray-700 hover:text-[#03b8ff] text-left"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 1 1 0-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.845 20.845 0 0 1-1.44-4.282m3.102.069a18.03 18.03 0 0 1-.59-4.59c0-1.586.205-3.124.59-4.59m0 9.18a23.848 23.848 0 0 1 8.835 2.535M10.34 6.66a23.847 23.847 0 0 0 8.835-2.535m0 0A23.74 23.74 0 0 0 18.795 3m.38 1.125a23.91 23.91 0 0 1 1.014 5.395m-1.014 8.855c-.118.38-.245.754-.38 1.125m.38-1.125a23.91 23.91 0 0 0 1.014-5.395m0-3.46c.495.413.811 1.035.811 1.73 0 .695-.316 1.317-.811 1.73m0-3.46a24.347 24.347 0 0 1 0 3.46" />
                    </svg>
                ...
                @if(Auth::user()->isResearcher())
                    ...
                    <button 
                        ...
                        class="flex items-center {{ $navLinkClass(request()->is('surveys/create*')) }} cursor-pointer hover:text-[#03b8ff] text-left"
                    >
    ...
    <!-- Mobile Menu Overlay (closes menu on click) - NOW OUTSIDE NAV -->
    <div 
        x-show="mobileMenuOpen" 
        @click="toggleMenu()"
