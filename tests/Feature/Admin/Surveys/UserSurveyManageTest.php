...
it('dispatches event after super admin status update', function () {
    Auth::login($this->superAdmin);
...
it('dispatches event after institution admin status update', function () {
    Auth::login($this->institutionAdmin);
...
// Lock Reason Tests
it('stores lock reason when locking survey via super admin', function () {
    Auth::login($this->superAdmin);
...
it('stores lock reason when locking survey via institution admin', function () {
    Auth::login($this->institutionAdmin);
    
    $lockReason = 'Policy violation detected';
...
it('clears lock reason when unlocking survey via super admin', function () {
...
it('clears lock reason when unlocking survey via institution admin', function () {
    // First lock with reason
    $this->survey->update([
        'is_locked' => true,
        'lock_reason' => 'Test reason'
...
it('super admin can view archived survey details', function () {
    $this->survey->delete();
...
it('institution admin can view archived survey details', function () {
    $this->survey->delete();
...
// Survey Information Display Tests
it('displays survey information correctly in super admin modal', function () {
    Auth::login($this->superAdmin);
...
it('displays survey information correctly in institution admin modal', function () {
    Auth::login($this->institutionAdmin);
...
// Survey Tags Display Tests
it('displays survey tags in modal', function () {
    Auth::login($this->superAdmin);
    
    Livewire::test(UserSurveyViewModal::class, ['surveyId' => $this->survey->id])
        ->assertSee('General Tags')
        ->assertSee('Institution Tags');
});
